...
Create plots showing the number of fatalities by year and the number of killer tornadoes by year.
```{r}
...
Seasonal fatality rates
```{r}
df <- data.frame(KillerTorn.sf) %>%
  group_by(mo == 12 | mo == 1 | mo == 2) %>%
  summarize(PCfatRate = sum(fat)/sum(TotalPp),
            PHUfatRate = sum(fat)/sum(TtlHsnU))

df <- data.frame(KillerTorn.sf) %>%
  group_by(mo == 3 | mo == 4 | mo == 5) %>%
  summarize(PCfatRate = sum(fat)/sum(TotalPp),
            PHUfatRate = sum(fat)/sum(TtlHsnU))

df <- data.frame(KillerTorn.sf) %>%
  group_by(mo == 6 | mo == 7 | mo == 8) %>%
  summarize(PCfatRate = sum(fat)/sum(TotalPp),
            PHUfatRate = sum(fat)/sum(TtlHsnU))

df <- data.frame(KillerTorn.sf) %>%
  group_by(mo == 9 | mo == 10 | mo == 11) %>%
  summarize(PCfatRate = sum(fat)/sum(TotalPp),
            PHUfatRate = sum(fat)/sum(TtlHsnU))
```
...
C = ggplot(df, aes(x = Hour, y = PCfatRate)) +
  ...
  scale_x_continuous(breaks = c(0, 3, 6, 9, 12, 15, 18, 21), labels = c("12 AM", "3 AM", "6 AM", "9 AM", "12 PM", "3 PM", "6 PM", "9 PM")) +
  xlab("") +
  scale_y_continuous(labels = label_percent(accuracy = .01, scale = 100)) +
  ylab("Per-Capita Fatality Rate") +
  theme_minimal(base_size = 9.5) +
...
multiplot(A, C, B, D, cols = 2)
```
*Export as 7.25 x 6.75 portrait*
...
### A Geographic Distribution of Tornado Fatalities ###
...
B <- tm_shape(states, projection ="+init=epsg:2163") +
  ...
  tm_fill("nT",
          ...
          labels = c("1 to 2", "2 to 4", "4 to 6", "6 to 8", "> 8"),
          palette = "Reds", title = "") +
  tm_borders(alpha = 0.2) +
  tm_format('World', legend.position = c("left", "bottom"),
            attr.position = c("left", "bottom"),
...
out = aw_interpolate(sfT, 
               ...
               source = DJF, 
               sid = "ID",
               extensive = c("PerTornPerCountyFat"),
               weight = "sum", 
               output = "sf")
...
Try to make this plot in rasters
```{r}
Areas$ID = 1:nrow(Areas)
sfT$ID2 = 1:nrow(sfT)
...
```{r}
fat_raster <- rasterize(out, r, out$Fat, fun=max)

values(fat_raster) = log10(values(fat_raster))

A = tm_shape(states, projection ="+init=epsg:2163") +
  tm_borders(alpha = 0.2) +
  tm_fill(col = "grey94") +
  tm_shape(fat_raster, projection ="+init=epsg:2163") + 
  tm_raster(breaks = c(0, .5, 1, 1.5, 2, 2.25),
          labels = c("1 to 16", "16 to 27", "27 to 45", "45 to 74", "> 74"),
          palette = "Reds", title = "") +
  tm_shape(states, projection ="+init=epsg:2163", is.master = TRUE) +
  tm_borders(alpha = 0.2) +
  tm_format('World', legend.position = c("left", "bottom"),
            attr.position = c("left", "bottom"),
            legend.frame = FALSE) +
...
count_raster <- rasterize(GridRates, r, GridRates$nT, fun=max)
```
...
B = tm_shape(states, projection ="+init=epsg:2163") +
  tm_borders(alpha = 0.2) +
  tm_fill(col = "grey94") +
  tm_shape(count_raster, projection ="+init=epsg:2163") + 
  tm_raster(breaks = c(0, 2, 4, 6, 8, 10),
          labels = c("1 to 2", "2 to 4", "4 to 6", "6 to 8", "> 8"),
          palette = "Reds", title = "") +
  tm_shape(states, projection ="+init=epsg:2163", is.master = TRUE) +
  tm_borders(alpha = 0.2) +
...
```{r}
PHfat_raster <- rasterize(out, r, out$PFat, fun=max)

values(PHfat_raster) = log10(values(PHfat_raster))

C <- tm_shape(states, projection ="+init=epsg:2163") +
  tm_borders(alpha = 0.2) +
  tm_fill(col = "grey94") +
  tm_shape(PHfat_raster, projection ="+init=epsg:2163") + 
  tm_raster(breaks = c(0, .5, 1, 1.25, 1.5, 2),
          labels = c("1 to 16", "16 to 27", "27 to 35", "35 to 48", "> 48"),
          palette = "Reds", title = "") +
  tm_shape(states, projection ="+init=epsg:2163", is.master = TRUE) +
  tm_borders(alpha = 0.2) +
...
Mobile Home Fatalities
```{r}
MHfat_raster <- rasterize(out, r, out$MHFat, fun=max)

values(MHfat_raster) = log10(values(MHfat_raster))

D <- tm_shape(states, projection ="+init=epsg:2163") +
  tm_borders(alpha = 0.2) +
  tm_fill(col = "grey94") +
  tm_shape(MHfat_raster, projection ="+init=epsg:2163") + 
  tm_raster(breaks = c(0, .5, 1, 1.25, 1.5, 2),
          labels = c("1 to 16", "16 to 27", "27 to 35", "35 to 48", "> 48"),
          palette = "Reds", title = "") +
  tm_shape(states, projection ="+init=epsg:2163", is.master = TRUE) +
...
Export (.jpeg) at 650 width and maintain aspect ratio.
...
```{r}
Dayfat_raster <- rasterize(out, r, out$Fat, fun=max)

values(Dayfat_raster) = log10(values(Dayfat_raster))

A <- tm_shape(states, projection ="+init=epsg:2163") +
  tm_borders(alpha = 0.2) +
  tm_fill(col = "grey94") +
  tm_shape(Dayfat_raster, projection ="+init=epsg:2163") + 
  tm_raster(breaks = c(0, .5, 1, 1.5, 2, 2.25),
          labels = c("1 to 16", "16 to 27", "27 to 45", "45 to 74", "> 74"),
          palette = "Reds", title = "") +
  tm_shape(states, projection ="+init=epsg:2163", is.master = TRUE) +
...
```{r}
Darkfat_raster <- rasterize(out, r, out$Fat, fun=max)

values(Darkfat_raster) = log10(values(Darkfat_raster))

B <- tm_shape(states, projection ="+init=epsg:2163") +
  tm_borders(alpha = 0.2) +
  tm_fill(col = "grey94") +
  tm_shape(Darkfat_raster, projection ="+init=epsg:2163") + 
  tm_raster(breaks = c(0, .5, 1, 1.5, 2, 2.25),
          labels = c("1 to 16", "16 to 27", "27 to 45", "45 to 74", "> 74"),
          palette = "Reds", title = "") +
  tm_shape(states, projection ="+init=epsg:2163", is.master = TRUE) +
...
```{r}
DJF_raster <- rasterize(out, r, out$Fat, fun=max)

values(DJF_raster) = log10(values(DJF_raster))

A <- tm_shape(states, projection ="+init=epsg:2163") +
  tm_borders(alpha = 0.2) +
  tm_fill(col = "grey94") +
  tm_shape(DJF_raster, projection ="+init=epsg:2163") + 
  tm_raster(breaks = c(0, .5, 1, 1.5, 2, 2.25),
          labels = c("1 to 16", "16 to 27", "27 to 45", "45 to 74", "> 74"),
          palette = "Reds", title = "") +
  tm_shape(states, projection ="+init=epsg:2163", is.master = TRUE) +
...
```{r}
MAM_raster <- rasterize(out, r, out$Fat, fun=max)

values(MAM_raster) = log10(values(MAM_raster))

B <- tm_shape(states, projection ="+init=epsg:2163") +
  tm_borders(alpha = 0.2) +
  tm_fill(col = "grey94") +
  tm_shape(MAM_raster, projection ="+init=epsg:2163") + 
  tm_raster(breaks = c(0, .5, 1, 1.5, 2, 2.25),
          labels = c("1 to 16", "16 to 27", "27 to 45", "45 to 74", "> 74"),
          palette = "Reds", title = "") +
  tm_shape(states, projection ="+init=epsg:2163", is.master = TRUE) +
...
```{r}
JJA_raster <- rasterize(out, r, out$Fat, fun=max)

values(JJA_raster) = log10(values(JJA_raster))

C <- tm_shape(states, projection ="+init=epsg:2163") +
  tm_borders(alpha = 0.2) +
  tm_fill(col = "grey94") +
  tm_shape(JJA_raster, projection ="+init=epsg:2163") + 
  tm_raster(breaks = c(0, .5, 1, 1.5, 2, 2.25),
          labels = c("1 to 16", "16 to 27", "27 to 45", "45 to 74", "> 74"),
          palette = "Reds", title = "") +
  tm_shape(states, projection ="+init=epsg:2163", is.master = TRUE) +
...
```{r}
SON_raster <- rasterize(out, r, out$Fat, fun=max)

values(SON_raster) = log10(values(SON_raster))

D <- tm_shape(states, projection ="+init=epsg:2163") +
  tm_borders(alpha = 0.2) +
  tm_fill(col = "grey94") +
  tm_shape(SON_raster, projection ="+init=epsg:2163") + 
  tm_raster(breaks = c(0, .5, 1, 1.5, 2, 2.25),
          labels = c("1 to 16", "16 to 27", "27 to 45", "45 to 74", "> 74"),
          palette = "Reds", title = "") +
  tm_shape(states, projection ="+init=epsg:2163", is.master = TRUE) +
